...
import { gridCSVGenerator } from "./grid-csv.js";
...
class LiquibaseGenerator {
  ...
  generateChangesetDefinition(
    ...
    let xmlContent = this.generateChangesetXml(
      change,
      changesetId,
      author,
      targetApplication,
    ...
    if (change.type === "CREATE_TABLE") {
      ...
      if (Array.isArray(payload.seedData) && payload.seedData.length > 0) {
        const seedSqlPath = this.generateSeedSqlFileName(
          payload,
          changesetId,
          targetApplication,
          targetSprint,
        );
        const seedSqlContent = this.generateSeedDataSQL(payload);

        sqlFiles = [
          ...(sqlFiles || []),
          { path: seedSqlPath, content: seedSqlContent },
        ];
        sqlFilePath = sqlFilePath || seedSqlPath;
        sqlFileContent = sqlFileContent || seedSqlContent;
        xmlContent = this.injectSqlFileReference(xmlContent, seedSqlPath);
      }
    }
  ...
  private generateSeedSqlFileName(
    payload: CreateTablePayload,
    changesetId: string,
    application: string,
    sprint: string,
  ): string {
    return `${application}/${sprint}/seed_${payload.tableName}_${changesetId}.sql`;
  }

  private generateSeedDataSQL(payload: CreateTablePayload): string {
    const schema = payload.schema || "public";
    const tableName = `"${schema}"."${payload.tableName}"`;
    const columnOrder = payload.columns.map((column) => column.name);
    const statements: string[] = [
      "-- Generated by Liquibase Migration Tool",
      "",
    ];

    for (const row of payload.seedData || []) {
      const rowColumns = columnOrder.filter((columnName) =>
        Object.prototype.hasOwnProperty.call(row, columnName),
      );
      const values = rowColumns.map((columnName) =>
        this.formatSeedSqlValue(row[columnName]),
      );
  ...
  private formatSeedSqlValue(value: string | number | boolean | null): string {
    if (value === null) {
      return "NULL";
    }
  ...
  private injectSqlFileReference(
    xmlContent: string,
    sqlFilePath: string,
  ): string {
    const sqlFileTag = `        <sqlFile path="${this.quoteXmlAttribute(sqlFilePath)}" relativeToChangelogFile="true"/>`;
